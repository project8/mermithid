...
#                         '18.000']
        ...
        z = np.sqrt(1 - w**2)*np.cos(phi)
        y = np.sqrt(1 - w**2 - z**2)*np.cos(theta)
        x = np.sqrt(1 - w**2 - z**2)*np.sin(theta)
        H2_fraction_list = w**2
        He_fraction_list = z**2
        Ar_fraction_list = y**2
        Kr_fraction_list = x**2
        i = 0
        for H2_fraction, He_fraction, Ar_fraction, Kr_fraction in zip(H2_fraction_list, He_fraction_list, Ar_fraction_list, Kr_fraction_list):
            ...
            logger.info('{} {} {} {}'.format(H2_fraction, He_fraction, Ar_fraction, H2_fraction+ He_fraction+ Ar_fraction + Kr_fraction))
            ...
            plt.savefig('/home/ys633/lineshape_fitting/plots/fit_October_FTC_with_max_snr_14.300_factor_{}_gas_fraction_variation_sphere_{}.png'.format(f, i))# March_FTC
            output_dict['max snr 14.300 factor {} gas fraction variation sphere {} H2 fraction {} He fraction {} Ar fraction {}'.format(f, i, H2_fraction, He_fraction, Ar_fraction)] = results
            i+=1
        np.save('/home/ys633/lineshape_fitting/mermithid_share/october_max_snr_14.300_factor_0.4955_gas_composition_variation_sphere.npy', output_dict)
