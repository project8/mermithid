...
class ComplexLineShapeTests(unittest.TestCase):
    ...
    def test_complex_lineshape(self):
        ...
        complexLineShape_config = {
            ...
            'fixed_parameter_names': ['survival probability', 'H2 scatter fraction', 'He scatter fraction', 'Ar scatter fraction'], #, 'width scale factor', 'H2 scatter fraction', 'He scatter fraction', 'Ar scatter fraction'
            'fixed_parameter_values': [1.0, (0.233+0.913)/2, 0.674/2, (0.051+0.104)/2 ],   #[1.0, 1.0, 0.886, 0.02, 0.06] 1.0, (0.233+0.913)/2, 0.674/2, (0.051+0.104)/2  
        ...
        b.Run()
        data = b.data
        freq_data_dict = np.load('/home/ys633/lineshape_fitting/mermithid_share/scatter_amplitude_single_deep_trap/Kr_paper_data_standard_candle.npy', allow_pickle = True).item()
        
        data_events = {}
        data_events['StartFrequency'] = np.array(freq_data_dict['start_freq'])
...
#         data['StartFrequency'] = np.array(start_freqs_vs_fss['0.0']) - 1.40812680e+09 + 50e6
        ...
        complexLineShape.data = data_events
        logger.info(data_events)
...
#        for directory in [directories[0]]:
        complexLineShape_config['path_to_ins_resolution_data_txt'] = '/home/ys633/lineshape_fitting/mermithid_share/scatter_amplitude_single_deep_trap/resolution-track-length/FTC-March-1.9e-4/res_cf15.5_trap3.txt'#'/home/ys633/lineshape_fitting/mermithid_share/averaged_resolutions/averaged_october_resolution.txt'
        #         if i == 10:
...
#        complexLineShape_config['scatter_fractions_for_gases'] = [0.817, 0.07, 0.08]
        ...
        plt.savefig('/home/ys633/lineshape_fitting/plots/single_deep_trap_3_fit_scaling_factor_new_gas_fraction_single_trap_res_Kr_paper_standard_candle_gaussian_res_fit_c.png')# March_FTC
        output_dict['trap 3 cf 15.5'] = results
        np.save('/home/ys633/lineshape_fitting/mermithid_share/single_deep_trap_3_fit_scaling_factor_new_gas_fraction_single_trap_res_Kr_paper_standard_candle_gaussian_res_fit_c.npy', output_dict)
#             time.sleep(600)
