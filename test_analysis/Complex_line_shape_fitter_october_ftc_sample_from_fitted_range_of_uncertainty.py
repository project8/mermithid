...
class ComplexLineShapeTests(unittest.TestCase):
    ...
    def test_complex_lineshape(self):
        ...
        complexLineShape_config = {
            ...
            'fixed_parameter_names': ['survival probability', 'width scale factor', 'scatter peak ratio param c', 'H2 scatter fraction', 'He scatter fraction', 'Ar scatter fraction'], #, 'width scale factor', 'H2 scatter fraction', 'He scatter fraction', 'Ar scatter fraction'
...
#         data['StartFrequency'] = np.array(start_freqs_vs_fss['0.0']) - 1.40812680e+09 + 50e6
        ...
        path_to_fit_result = '/home/ys633/lineshape_fitting/mermithid_share/october_max_snr_14.300_factor_0_new_gas_fraction.npy'
        fit_result = np.load(path_to_fit_result, allow_pickle = True).item()
        q_central = fit_result['october max snr 14.300']['scatter_peak_ratio_q_fit']
        q_uncertainty = fit_result['october max snr 14.300']['scatter_peak_ratio_q_fit_err']
        q_array = np.linspace(q_central - q_uncertainty, q_central + q_uncertainty, 30)
        f = 0
        output_dict = {}
...
#            complexLineShape_config['path_to_ins_resolution_data_txt'] = '/host/October_FTC_resolution/all_res_cf14.300.txt'
        ...
        for q in q_array:
            ...
            complexLineShape_config['fixed_parameter_values'] = [1.0, 1.0, q, (0.233+0.913)/2, 0.674/2, (0.051+0.104)/2]
    ...
    #        complexLineShape_config['scatter_fractions_for_gases'] = [0.817, 0.07, 0.08]
            ...
            plt.savefig('/home/ys633/lineshape_fitting/plots/fit_October_FTC_with_new_gas_fraction_factor_0_sample_from_fitted_range_of_uncertainty_q_{}.png'.format(q))# March_FTC
            output_dict['october max snr 14.300 with q {}'.format(q)] = results
        np.save('/home/ys633/lineshape_fitting/mermithid_share/october_max_snr_14.300_factor_0_new_gas_fraction_sample_from_fitted_range_of_uncertainty.npy', output_dict)
#             time.sleep(600)
