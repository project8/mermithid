...
#                         '18.000']
        ...
        for f in [0.4626]:
            output_dict = {}
            for i in range(11):
                fixed_para_values = [1.0]

                complexLineShape_config['path_to_ins_resolution_data_txt'] = '/host/res_stat_March_ftc/res_stat_{}.txt'.format(i)
                if i == 10:
                    complexLineShape_config['path_to_ins_resolution_data_txt'] = '/host/March_FTC_resolution/all_res_cf15.300.txt'
                ...
                plt.savefig('/host/plots/fit_March_FTC_with_simulated_resolution_correlation_factor.png')# March_FTC
                output_dict['correlation factor res stat {}'.format(i)] = results
            np.save('/host/march_correlation_res_stat_for_factor_{:.5}.npy'.format(f), output_dict)
