...
class ComplexLineShapeTests(unittest.TestCase):
    ...
    def test_complex_lineshape(self):
        ...
        complexLineShape_config = {
            ...
            'fixed_parameter_values': [1.0, (0.233+0.913)/2, 0.674/2, (0.051+0.104)/2],   #[1.0, 1.0, 0.886, 0.02, 0.06]   
...
#         data['StartFrequency'] = np.array(start_freqs_vs_fss['0.0']) - 1.40812680e+09 + 50e6
    ...
    #        for directory in [directories[0]]:
            complexLineShape_config['path_to_ins_resolution_data_txt'] = '/home/ys633/lineshape_fitting/mermithid_share/October_FTC_resolution/all_res_cf{:.3f}.txt'.format(max_snr)
            #         if i == 10:
    ...
    #        complexLineShape_config['scatter_fractions_for_gases'] = [0.817, 0.07, 0.08]
            ...
            plt.savefig('/home/ys633/lineshape_fitting/plots/fit_October_FTC_max_snr_optimization_{:.3f}_new_gas_fraction.png'.format(max_snr))
            output_dict['october max snr {:.3f}'.format(max_snr)] = results
        np.save('/home/ys633/lineshape_fitting/mermithid_share/october_max_snr_optimizatoin_factor_0.4955_new_gas_fraction.npy', output_dict)
#             time.sleep(600)
