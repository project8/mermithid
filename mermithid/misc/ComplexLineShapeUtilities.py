...
def get_eloss_spec(e_loss, oscillator_strength, kr_17keV_line): #energies in eV
    ...
    argument_of_log = np.where(e_loss > 0, 4. * kinetic_en * e_loss / (e_rydberg**3.) , 1e-5)
    return np.where(e_loss>0 , 4.*np.pi*a0**2 * e_rydberg / (kinetic_en * e_loss) * oscillator_strength * np.log(argument_of_log), 0)
