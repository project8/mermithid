"""
To test mermithid.processors.DataGenerator4D.DataGenerator4D with numerical sampler.
It will take ~min to run.

command: python3 DataGenerator4D_numerical_sampler_test.py

Author: S. M. Lee
First Date: July 20, 2025
...
class DataGenerator4DTest(unittest.TestCase):
    ...
    def test_data_generation(self):
        ...
        specGen_config = {
            ...
            "r_bins": 140,
            "phi_bins": 360,
            # Source configuration: numerical
            "source_types": ["numerical"],
            "source_numerical_binned_mode": True,
            "source_numerical_ke_rate_path": "./DataGenerator4D_helpers/numerical_sampler/energy_two_gaussian.npz",
            ...
            "spatial_model": "numerical",
            "spatial_numerical_hist_path": "./DataGenerator4D_helpers/numerical_sampler/spatial_box.npz",
            "spatial_numerical_apply_trapping_efficiency": True,
        ...
        ax_r_phi: plt.Axes = fig_response.add_subplot(gs_response[8:13, 2:])
        ...
        phi_start_all = np.concatenate(
            [results[i]["phi_start"] for i in range(len(results))]
        )
        ...
        # r-phi plots
        ax_r_phi.scatter(
            r_start_all * np.cos(phi_start_all),
            r_start_all * np.sin(phi_start_all),
            color="black",
            s=1,
            alpha=0.01,
            label="Trapped " + r"$e^{-}$" + " positions",
        )

        ax_r_phi.set_xlim(-var_items[2]["max"], var_items[2]["max"])
        ax_r_phi.set_ylim(-var_items[2]["max"], var_items[2]["max"])
        ax_r_phi.set_xlabel(r"$x$ [m]")
        ax_r_phi.set_ylabel(r"$y$ [m]")
        ax_r_phi.set_xticks([-var_items[2]["max"], 0, var_items[2]["max"]])
        ax_r_phi.set_yticks([-var_items[2]["max"], 0, var_items[2]["max"]])
        ax_r_phi.set_xticks(np.arange(-var_items[2]["max"], var_items[2]["max"] + 0.0005, 0.001), minor=True)
        ax_r_phi.set_yticks(np.arange(-var_items[2]["max"], var_items[2]["max"] + 0.0005, 0.001), minor=True)
        ax_r_phi.grid(True, which="both", alpha=0.2)
        ax_r_phi.legend()
        ...
        plt.savefig("DataGenerator4D_numerical_sampler_test.png", dpi=300)
